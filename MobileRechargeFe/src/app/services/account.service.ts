...
export class AccountService {
    ...
    async active(email: string, token: string) {
        var value = this.httpClient.get(this.BASE_URL + 'active/' + email + '/' + token);
        return await lastValueFrom(value);
    }
    ...
    async changePass(email: string, token: string, password: string) {
        var value = this.httpClient.get(this.BASE_URL + 'changepass/' + email + '/' + token +'/'+password);
        return await lastValueFrom(value);
    }

    async login(email: string, password: string) {
        var value = this.httpClient.get(this.BASE_URL + 'login/' + email + '/' + password);
        return await lastValueFrom(value);
    }
}